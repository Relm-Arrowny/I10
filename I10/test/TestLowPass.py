...
for i in range (0,1000):
    twave =   WaveProcessing().waveGen(10000, 10.0, np.random.random()*1.0+1.0, np.random.random()*500.0,
                                        np.random.random()*10.0, np.random.random()*360)
    wav = np.add(twave , wav)
plt.subplot(211)
plt.plot(wav[0],wav[1])

wavLowpass = WaveProcessing().fftLowpassFilter(wav[1], 0, 10.0/10000.0)
